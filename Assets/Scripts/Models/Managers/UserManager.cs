...
namespace ChineseCheckers
{
    public static class UserManager
    {
        ...
        static void FindOptimalPlay(CompPlayer player, out Node target, out Piece selectedPiece)
        {
            ...
            List<Board> listOfPaths = CompPlayer.FindAllPossiblePaths(2, player, BoardManager.board, false);
            ...
            List<Piece> boardPieces = player.CachedPieces;
            ...
            foreach (var piece in bestBoardPieces)
            {
                ...
                bestBoard[pos.x, pos.y].Object.HighlightNode(Color.green, true);
                if (BoardManager.board[pos.x, pos.y].StoredPiece.BelongsTo == Team.Empty)
                {
            ...
            foreach (var piece in boardPieces)
            {
                ...
                Vector2Int pos = piece.CurrentPosition;
                if (bestBoard[pos.x, pos.y].StoredPiece == null) continue;
                if (bestBoard[pos.x, pos.y].StoredPiece.BelongsTo == Team.Empty)
                {
            ...
            Debug.Log($"Has found optimal play! Selected Piece: {_selectedPiece}/ Target: {_target}", player);
