...
namespace ChineseCheckers
{
    ...
    public class Board
    {
        ...
        public Node[,] Clone()
        {
            ...
            Node[,] newBoard = new Node[board.GetLength(0), board.GetLength(1)];
            for (int y = 0; y < board.GetLength(0); y++)
            {
                for (int x = 0; x < board.GetLength(1); x++)
                {
                    Node newNode = null;
                    Debug.Log(board[y, x]);
                    if (board[y, x].StoredPiece != null)
                    {

                        Piece newPiece = new Piece(board[y, x].StoredPiece);
                        newNode = new Node(board[y, x], newPiece);
                        newBoard[y, x] = newNode;
                        continue;
                    }
                    newNode = new Node(board[y, x], null);
                    newBoard[y, x] = newNode;
                }
            }
            return newBoard;
    ...
    public abstract class BoardManager
    {
        ...
        public static Node[,] originalBoard;
        ...
        public static Node GetValidNode(Node source, List<Node> validMoves, int index)
        {
            ...
            Vector2Int pos = source.CurrentPosition, boardDir = Node.DirectionInBoard(pos, index), newPos = pos + boardDir;
